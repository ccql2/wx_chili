...
try:
    with connection.cursor() as cursor:
        ...
        sql = "SELECT * FROM chili_5pq LIMIT 1"
        cursor.execute(sql)
        record = cursor.fetchone()

        # 提取记录中的关键信息
        region_name = record['region_name'] if record else '第五片区'
        region_image_url = record['region_image_url'] if record else ''
        operator = record['operator'] if record else '郑十一'
        operation_time = record['operation_time'] if record else datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        anti_counterfeiting_code = record['anti_counterfeiting_code'] if record else 'MNO234'
        region_leader = record['region_leader'] if record else '陈十二'
        fertilizers_used = record['fertilizers_used'] if record else '复合肥,有机肥'
        farming_activities = record['farming_activities'] if record else '灌溉'
        watering_fertilizing_pesticide_logs = record['watering_fertilizing_pesticide_logs'] if record else '2023-04-10 08:30 郑十一灌溉; 2023-04-25 13:00 陈十二检查土壤'
        ...
        # 将HTML内容保存到文件中
        with open("fifth_chili.html", "w", encoding="utf-8") as file:

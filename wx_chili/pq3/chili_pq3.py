...
try:
    with connection.cursor() as cursor:
        ...
        sql = "SELECT * FROM chili_3pq LIMIT 1"
        cursor.execute(sql)
        record = cursor.fetchone()

        # 提取记录中的关键信息
        region_name = record['region_name'] if record else '第三片区'
        region_image_url = record['region_image_url'] if record else ''
        operator = record['operator'] if record else '李七'
        operation_time = record['operation_time'] if record else datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        anti_counterfeiting_code = record['anti_counterfeiting_code'] if record else 'FGH678'
        region_leader = record['region_leader'] if record else '孙八'
        fertilizers_used = record['fertilizers_used'] if record else '磷酸二铵,钾肥'
        farming_activities = record['farming_activities'] if record else '除草'
        watering_fertilizing_pesticide_logs = record['watering_fertilizing_pesticide_logs'] if record else '2023-04-05 09:00 李七除草; 2023-04-20 10:00 孙八施肥'
        ...
        # 将HTML内容保存到文件中
        with open("third_chili.html", "w", encoding="utf-8") as file:

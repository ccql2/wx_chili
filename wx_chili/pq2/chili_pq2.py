...
try:
    with connection.cursor() as cursor:
        ...
        sql = "SELECT * FROM chili_2pq LIMIT 1"
        cursor.execute(sql)
        record = cursor.fetchone()

        # 提取记录中的关键信息
        region_name = record['region_name'] if record else '第二片区'
        region_image_url = record['region_image_url'] if record else ''
        operator = record['operator'] if record else '王五'
        operation_time = record['operation_time'] if record else datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        anti_counterfeiting_code = record['anti_counterfeiting_code'] if record else 'CDE345'
        region_leader = record['region_leader'] if record else '赵六'
        fertilizers_used = record['fertilizers_used'] if record else '腐殖酸液态有机肥'
        farming_activities = record['farming_activities'] if record else '播种'
        watering_fertilizing_pesticide_logs = record['watering_fertilizing_pesticide_logs'] if record else '2023-04-01 08:00 王五浇水; 2023-04-15 14:00 赵六施肥'
        ...
        # 将HTML内容保存到文件中
        with open("second_chili.html", "w", encoding="utf-8") as file:
